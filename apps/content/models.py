...
class Audio(TimeBaseModel):
    ...
    audio = FileField(_('audio'), upload_to=audio_upload_path)
    ...
    def save(self, force_insert=False, force_update=False, using=None, update_fields=None):
        ...
        if os.path.splitext(audio_name)[1] != '.mp3':
            upload_path = os.path.dirname(self.audio.path)
            audio_filename = os.path.basename(audio_name)
            audio_path = os.path.join(upload_path, audio_filename)
            sound = AudioSegment.from_file(audio_path)
            mp3_path = os.path.splitext(audio_filename)[0] + '.mp3'
            mp3_full_path = os.path.join(upload_path, mp3_path)
            sound.export(mp3_full_path, format="mp3")
            self.audio.name = self.audio.name[:10] + mp3_path
